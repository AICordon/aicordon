...
def test_a_clean_request_passes_through_as_the_same_object() -> None:
    ...
    assert PromptInjectionValidator(mode="fail").invoke(value) is value


def test_a_flagged_request_raises_in_fail_mode(attack: str) -> None:
    with pytest.raises(InjectionFound) as caught:
        PromptInjectionValidator(mode="fail").invoke(ChatPromptValue(messages=[HumanMessage(content=attack)]))
...
def test_annotate_marks_the_messages_and_keeps_the_type(attack: str) -> None:
    ...
    out = PromptInjectionValidator(mode="passthrough").invoke(value)
...
def test_the_default_marks_and_does_not_raise(attack: str) -> None:
    """A link added to a chain must not start raising on the caller's traffic. The default reads
    and records; raising is a mode somebody chose."""
    value = ChatPromptValue(messages=[HumanMessage(content=attack)])
    out = PromptInjectionValidator().invoke(value)
    assert out.messages[0].additional_kwargs["picket_flagged"] is True
...
def test_a_bare_string_is_read_as_the_request(attack: str) -> None:
    ...
    with pytest.raises(InjectionFound):
        PromptInjectionValidator(mode="fail").invoke(attack)
    assert PromptInjectionValidator(mode="fail").invoke("hello") == "hello"


def test_a_list_of_messages_is_read_as_well(attack: str) -> None:
    with pytest.raises(InjectionFound):
        PromptInjectionValidator(mode="fail").invoke([("system", "Be helpful."), ("user", attack)])


@pytest.mark.asyncio
async def test_the_async_form_decides_the_same(attack: str) -> None:
    with pytest.raises(InjectionFound):
        await PromptInjectionValidator(mode="fail").ainvoke(ChatPromptValue(
